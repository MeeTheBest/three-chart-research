...
export default function Home() {
  ...
  async function startSession(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const data = await request("/sessions", { method: "POST", body: JSON.stringify({ date: form.get("date"), time: form.get("time"), place: form.get("place"), gender: form.get("gender"), fold: form.get("fold"), baziTimeStandard: trueSolar ? "true_solar" : "civil" }) });
      setPartialReports({});
  ...
  async function analyze(system: SystemKey) {
    if (results[system]) { document.getElementById(`report-${system}`)?.scrollIntoView({ behavior: "smooth", block: "start" }); return; }
    if (!sessionId) return;
    setBusy(system);
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      <form className="birth-form" onSubmit={startSession}><div className="section-heading"><div><h2>出生信息</h2></div></div><div className="form-grid">
        ...
        <fieldset className="gender-field"><legend>性别</legend><div>{["女", "男"].map((value) => <label key={value}><input type="radio" name="gender" value={value} defaultChecked={value === "女"} /><span>{value}</span></label>)}</div></fieldset>
        <details><summary>时间补充说明</summary><p>保留原始钟表时间。真太阳时使用城市参考坐标计算，仅影响八字的日、时口径；不会改变实际出生瞬间。</p><label>仅当时钟回拨、同一时间出现两次时选择<select name="fold" defaultValue=""><option value="">不适用 / 尚未确认</option><option value="0">第一次出现</option><option value="1">第二次出现</option></select></label><p>新版真太阳时采用天文算法；旧报告保留原结果，重新排盘才使用新版算法。</p></details>
      </div><label className="true-solar-option"><input type="checkbox" checked={trueSolar} onChange={(event) => setTrueSolar(event.target.checked)} />八字使用真太阳时校正</label><button className="primary" type="submit" disabled={!birthTime || !cityCode || busy !== null}>{busy === "integration" && !progress && !sessionId ? "准备中…" : sessionId ? "重新排盘 →" : "排盘 →"}</button></form>
