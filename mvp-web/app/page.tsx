...
export default function Home() {
  ...
  return <main className="shell">
    <header className="hero"><div className="hero-heading"><div><p className="eyebrow">RESEARCH MODE · 匿名临时会话</p><h1>三术命盘研究台</h1></div><button className="history-button" type="button" onClick={() => void openHistory()}>历史记录</button></div><p className="hero-copy">先独立分析，再比较一致与冲突。不输出统一人生结论，也不替代现实判断。</p></header>
    ...
    <section className="workspace">
      ...
      <section className={`comparison${comparisonReady ? " comparison-ready" : ""}`}><div className="section-heading"><span>03</span><div><h2>三术比较</h2><p>仅在三项单术都已冻结后解锁。</p></div></div><div className="comparison-body"><span className="lock">{comparisonReady ? "✓" : "⌁"}</span><div><strong>比较层，不是第四套命理</strong><p>{comparisonReady ? "三项单术已冻结，可比较一致、部分一致、冲突与不确定性。" : sessionId ? `尚需完成：${missingNames || "三项单术分析"}。` : "请先创建临时研究会话，再完成三项单术分析。"}</p></div><button type="button" disabled={!comparisonReady || busy !== null} onClick={compare}>{busy === "integration" ? "比较中…" : results.integration ? "查看比较结果" : "开始三术比较"}</button></div></section>
